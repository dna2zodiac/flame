...
export const DataClient = {
   ...
   Project: {
      ...
      GetFileContentsRaw: async (print_path) => {
         const req = await fetch(`/zoekt/${print_path}`);
      ...
      Search: async (query, n) => {
         n = n || 50;
         const req = await fetch(`/zoekt/search?q=${encodeURIComponent(query)}&num=${n}&format=json`);
         return await req.json();
      }, // Search
   }, // Project
   Topic: {
