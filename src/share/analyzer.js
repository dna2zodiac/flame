...
//const searchPlugin = require('./plugin/analyzer/search_indexer');
const printerPlugin = require('./plugin/analyzer/printer');
...
function AnalyzeProject(srcRoot, outDir, opt) {
   ...
   async function scanDfs(path, outFd) {
      try {
   ...
   /*sync*/ function getMetaDirname(hash) {
      let parts = [];
   ...
   async function analyzeFileItem(obj) {
      const path = iPath.join(srcRoot, obj.p);
   ...
   async function updateFileItem(obj) {
      const path = iPath.join(srcRoot, obj.p);
   ...
   async function removeFileItem(obj) {
      const path = iPath.join(srcRoot, obj.p);
      const outHashDir = getMetaDirname(obj.h);
