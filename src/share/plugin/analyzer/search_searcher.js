const { FsReaddir, FsExists, FsReadFile } = require('../../file_op');
const { LineReader } = require('../../large_file');
const { getHashDir, getTrigramDir } = require('./search_indexer');
...
async function TrigramSearch(query, metaRoot, opt) {
...
async function TrigramSearchCandidate(query, metaRoot, opt) {
   metaRoot = iPath.resolve(metaRoot || '.');
   opt = opt || {};
   const r: any[] = [];
   const N = opt.N || 1000;
...
async function readRiHashIdList(riP, r, N) {
...
async function transformHashIdToPath(r, metaRoot) {
   const items = [];
   const indexDir = iPath.join(metaRoot, '_index');
