...
const {LineReader} = require('../../large_file');
const {/*GenInMemWordIndex, */GenInMemTrigramIndex} = require('../../search/indexer');
...
function getHashDir(metaRoot, hash): string {
   let parts = [];
...
/* special export; friend = ./search_searcher */
function getTrigramDir(tri, rootDir) {
...
async function mergeIndexLine(targetP, line) {
   if (!line) return;
   let ix = line.indexOf(' ');
   if (ix < 0) return;
   const id = parseInt(line.substring(0, ix), 10);
...
async function removeIndexLine(targetP, id) {
...
async function PostProjectLv(metaRoot) {
